...
bool InputGeom::loadMesh(rcContext* ctx, rcMeshLoaderObj* inputMesh)
{
	if (NULL == inputMesh)
		return false;

	m_mesh = new rcMeshLoaderObj;
	m_mesh->m_verts = new float[inputMesh->getVertCount()*3];
	m_mesh->m_tris = new int[inputMesh->getTriCount()*3];

	strcpy(m_mesh->m_filename, inputMesh->m_filename);
	m_mesh->m_scale = inputMesh->m_scale;
	memcpy(m_mesh->m_verts, inputMesh->m_verts, inputMesh->getVertCount()*sizeof(float)*3);
	memcpy(m_mesh->m_tris, inputMesh->m_tris, inputMesh->getTriCount()*sizeof(int)*3);
	m_mesh->m_normals = 0;
	m_mesh->m_vertCount = inputMesh->getVertCount();
	m_mesh->m_triCount = inputMesh->getTriCount();
