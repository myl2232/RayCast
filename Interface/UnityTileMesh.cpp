...
public:
	...
	virtual void init(UnityNavMesh* sample)
	{
		m_sample = (UnityTileMesh*)sample; 
	}
...
void UnityTileMesh::saveAll(int** buffer, int* size)
{
	if (!m_navMesh) return;
	...
	m_saveBuffer.write(&header, sizeof(NavMeshSetHeader));
	...
	for (int i = 0; i < mesh->getMaxTiles(); ++i)
	{
		...
		m_saveBuffer.write(&tileHeader, sizeof(tileHeader));

		//fwrite(tile->data, tile->dataSize, 1, fp);
		m_saveBuffer.write(tile->data, tile->dataSize);
	}

	if (buffer)
		*buffer = (int*)m_saveBuffer.get();
	if (size)
		*size = m_saveBuffer.bufferSize();
}

dtNavMesh* UnityTileMesh::loadAll(const char* memory)
{
	if (!memory) return 0;
	...
	memcpy(&header, memory, sizeof(NavMeshSetHeader));
	memory += sizeof(NavMeshSetHeader);
// 	if (readLen != 1)
// 	{
// 		fclose(fp);
// 		return 0;
// 	}
	...
	for (int i = 0; i < header.numTiles; ++i)
	{
		...
		memcpy(&tileHeader, memory, sizeof(tileHeader));
		memory += sizeof(tileHeader);
		...
		memcpy(data, memory, tileHeader.dataSize);
		memory += tileHeader.dataSize;
// 		if (readLen != 1)
// 			return 0;
...
void UnityTileMesh::handleSettings(const NavMeshBuildSettings& params)
{
	UnityNavMesh::handleCommonSettings(params);
...
// 	
 	if (m_geom)
 	{
		...
		float worldTileSize = (rcMax(bmax[0] - bmin[0], bmax[2] - bmin[2]) / m_cellSize);
		m_tileSize = (worldTileSize<100) ? 100 : (worldTileSize>1000) ? 1000 : worldTileSize;
...
void UnityTileMesh::handleTools()
{
...
void UnityTileMesh::handleDebugMode()
{
...
void UnityTileMesh::handleRender(duDebugDraw* dd, bool showDetail)
{
	UnityNavMesh::handleRender(dd, showDetail);
...
void UnityTileMesh::handleRenderOverlay(double* proj, double* model, int* view)
{
// 	GLdouble x, y, z;
// 	
...
void UnityTileMesh::handleMeshChanged(class InputGeom* geom)
{
	UnityNavMesh::handleMeshChanged(geom);
...
bool UnityTileMesh::handleBuild()
{
	if (!m_geom || !m_geom->getMesh())
	{
...
void UnityTileMesh::buildTile(const float* pos)
{
	if (!m_geom) return;
	if (!m_navMesh) return;
...
void UnityTileMesh::getTilePos(const float* pos, int& tx, int& ty)
{
	if (!m_geom) return;
...
void UnityTileMesh::removeTile(const float* pos)
{
	if (!m_geom) return;
	if (!m_navMesh) return;
...
void UnityTileMesh::buildAllTiles()
{
	if (!m_geom) return;
	if (!m_navMesh) return;
...
void UnityTileMesh::removeAllTiles()
{
	const float* bmin = m_geom->getMeshBoundsMin();
	const float* bmax = m_geom->getMeshBoundsMax();
	int gw = 0, gh = 0;
...
unsigned char* UnityTileMesh::buildTileMesh(const int tx, const int ty, const float* bmin, const float* bmax, int& dataSize)
{
	...
	m_cfg.walkableHeight = (int)floorf(m_agentHeight / m_cfg.ch);
	m_cfg.walkableClimb = (int)ceilf(m_agentMaxClimb / m_cfg.ch);
	m_cfg.walkableRadius = (int)ceilf(m_agentRadius / m_cfg.cs);
	m_cfg.maxEdgeLen = 0;//(int)(m_edgeMaxLen / m_cellSize);
	m_cfg.maxSimplificationError = m_edgeMaxError;
	//m_cfg.minRegionArea = (int)rcSqr(m_regionMinSize);		// Note: area = size*size
	m_cfg.minRegionArea = (int)(m_regionMinSize/rcSqr(m_cellSize));
	...
	m_cfg.detailSampleDist = m_detailSampleDist;
	//m_cfg.detailSampleMaxError = m_cellHeight * m_detailSampleMaxError;
	m_cfg.detailSampleMaxError = m_cellHeight;
...
// 	{
// 		const char* msg = m_ctx->getLogText(i);
// 		if (msg)
