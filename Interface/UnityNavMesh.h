...
struct SampleTool
{
	virtual ~SampleTool() {}
	virtual int type() = 0;
	virtual void init(class UnityNavMesh* sample) = 0;
...
public:
	UnityNavMesh();
	virtual ~UnityNavMesh();

	dtStatus getPathFindStatus() { return m_pathFindStatus; }
	int getSmoothPathNum() { return m_nsmoothPath; }
	...
	void pathFind(const float* s, const float* e);
	...
	virtual void handleRenderdPolyBoundaries(duDebugDraw* dd);
	...
	void initToolStates(UnityNavMesh* sample);
	void resetToolStates();
	void renderToolStates();
	void renderOverlayToolStates(double* proj, double* model, int* view);

	void resetCommonSettings();
	void handleCommonSettings(const NavMeshBuildSettings& params);
};
