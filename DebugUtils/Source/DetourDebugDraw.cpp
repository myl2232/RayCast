							   ...
							   bool inner)
{
	...
	for (int i = 0; i < tile->header->polyCount; ++i)
	{
		...
		for (int j = 0, nj = (int)p->vertCount; j < nj; ++j)
		{
			...
			{
				const dtPolyDetailIndex* t = &tile->detailTris[(pd->triBase+k)*4];
				const float* tv[3];
				for (int m = 0; m < 3; ++m)
				{
					if (t[m] < p->vertCount)
...
void duDebugDrawNavMeshPolyBoundaries(duDebugDraw* dd, const dtNavMesh& mesh)
{
	if (!dd)
		return;
...
void duDebugDrawNavMeshPoly(duDebugDraw* dd, const dtNavMesh& mesh, dtPolyRef ref, const unsigned int col)
{
	...
	else
	{
		...
		{
			const dtPolyDetailIndex* t = &tile->detailTris[(pd->triBase+i)*4];
			for (int j = 0; j < 3; ++j)
			{
