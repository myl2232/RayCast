...
bool dtCreateNavMeshData(dtNavMeshCreateParams* params, unsigned char** outData, int* outDataSize)
{
	...
	const int detailTrisSize = dtAlign4(sizeof(dtPolyDetailIndex)*4*detailTriCount);
	...
	if (params->detailMeshes)
	{
		...
		for (int i = 0; i < params->polyCount; ++i)
		{
			...
			dtl.vertCount = (dtPolyDetailIndex)(ndv-nv);
			dtl.triBase = (unsigned int)params->detailMeshes[i*4+2];
			dtl.triCount = (dtPolyDetailIndex)params->detailMeshes[i*4+3];
		...
		// Store triangles.
		memcpy(navDTris, params->detailTris, sizeof(dtPolyDetailIndex)*4*params->detailTriCount);
	}
	else
	{
		...
		for (int i = 0; i < params->polyCount; ++i)
		{
			...
			dtl.triCount = (dtPolyDetailIndex)(nv-2);
			// Triangulate polygon (local indices).
			for (int j = 2; j < nv; ++j)
			{
				dtPolyDetailIndex* t = (dtPolyDetailIndex*)&navDTris[tbase*4];
				t[0] = 0;
