						   ...
						   rcPolyMeshDetail& dmesh)
{
	...
	dmesh.ntris = 0;
	dmesh.tris = (rcPolyMeshDetailIndex*)rcAlloc(sizeof(rcPolyMeshDetailIndex*)*tcap*4, RC_ALLOC_PERM);
	if (!dmesh.tris)
	{
	...
	for (int i = 0; i < mesh.npolys; ++i)
	{
		...
		if (dmesh.ntris+ntris > tcap)
		{
			while (dmesh.ntris+ntris > tcap)
				tcap += 256;
			rcPolyMeshDetailIndex* newt = (rcPolyMeshDetailIndex*)rcAlloc(sizeof(rcPolyMeshDetailIndex)*tcap*4, RC_ALLOC_PERM);
			if (!newt)
			{
			...
			if (dmesh.ntris)
				memcpy(newt, dmesh.tris, sizeof(rcPolyMeshDetailIndex)*4*dmesh.ntris);
			rcFree(dmesh.tris);
			dmesh.tris = newt;
		}
		for (int j = 0; j < ntris; ++j)
		{
			...
			dmesh.tris[dmesh.ntris*4+0] = (rcPolyMeshDetailIndex)t[0];
			dmesh.tris[dmesh.ntris*4+1] = (rcPolyMeshDetailIndex)t[1];
			dmesh.tris[dmesh.ntris*4+2] = (rcPolyMeshDetailIndex)t[2];
...
bool rcMergePolyMeshDetails(rcContext* ctx, rcPolyMeshDetail** meshes, const int nmeshes, rcPolyMeshDetail& mesh)
{
	...
	mesh.ntris = 0;
	mesh.tris = (rcPolyMeshDetailIndex*)rcAlloc(sizeof(rcPolyMeshDetailIndex)*maxTris*4, RC_ALLOC_PERM);
	if (!mesh.tris)
	{
